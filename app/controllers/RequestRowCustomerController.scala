...
class RequestRowCustomerController @Inject() (repo: RequestRowCustomerRepository, repoRequestRow: RequestRowRepository,
  repoProduct: ProductRepository, repoModule: ModuleRepository, repoDriver: CustomerRepository, repoCustomer: CustomerRepository,
  ...
  def addModule = Action.async { implicit request =>
    newForm.bindFromRequest.fold(
      ...
      res => {
        repo.create(
          res.requestRowId, res.productId, products(res.productId.toString()),
          res.customerId, modules(res.customerId.toString()),
          res.quantity, res.price, res.totalPrice, res.paid, res.credit,
          res.status, res.measureId, measures(res.measureId.toString()), moduleType, res.observation).map { resNew =>
            //repoModule.updateTotalDebt(res.customerId, res.credit);
  ...
  def getModuleById(moduleId: Long): Map[String, String] = {
    Await.result(repoModule.getById(moduleId).map {
      case res1 =>
        val cache = collection.mutable.Map[String, String]()
        res1.foreach { module =>
  ...
  def updateModulePost = Action.async { implicit request =>
    updateForm.bindFromRequest.fold(
      ...
      res => {
        repo.update(res.id, res.requestRowId, res.productId, products(res.productId.toString), res.customerId,
          modules(res.customerId.toString), res.quantity, res.price, res.totalPrice, res.paid,
          res.credit, res.status, res.measureId, measures(res.measureId.toString()),
  ...
  val searchModuleForm: Form[SearchModuleForm] = Form {
    mapping(
      "search" -> text)(SearchModuleForm.apply)(SearchModuleForm.unapply)
  }

  val searchDriverForm: Form[SearchDriverForm] = Form {
    mapping(
      "search" -> text)(SearchDriverForm.apply)(SearchDriverForm.unapply)
  }
    ...
    searchDriverForm.bindFromRequest.fold(
      errorForm => {
        Future.successful(Ok(views.html.requestRowDriver_add(new MyDeadboltHandler, parentId, searchDriverForm,
          newDriverForm, requestRows, products, customers, measures)))
      },
      res => {
        repoDriver.searchCustomer(res.search).map { resDrivers =>
          val cache = collection.mutable.Map[String, String]()
