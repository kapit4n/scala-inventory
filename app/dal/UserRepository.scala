...
class UserRepository @Inject() (dbConfigProvider: DatabaseConfigProvider, repoLog: LogEntryRepository)(implicit ec: ExecutionContext) {
  ...
  private class UsersTable(tag: Tag) extends Table[User](tag, "user") {
    ...
    def * = (id, name, carnet, telefono, direccion, Salary, type_1, login, password) <> ((User.apply _).tupled, User.unapply)
  }
  ...
  def create(name: String, carnet: Int, telefono: Int, direccion: String,
    ...
    (tableQ.map(p => (p.name, p.carnet, p.telefono, p.direccion, p.Salary, p.type_1, p.login, p.password))
      returning tableQ.map(_.id)
      into ((nameAge, id) => User(id, nameAge._1, nameAge._2, nameAge._3, nameAge._4, nameAge._5, nameAge._6, nameAge._7, nameAge._8))) += (name, carnet, telefono, direccion, Salary, type_1, login, password)
  }
  ...
  def update(id: Long, name: String, carnet: Int, telefono: Int,
    direccion: String, Salary: Int, type_1: String,
